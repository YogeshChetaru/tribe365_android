...
public class Adapter_Admin_Report_Diagnostics_graph_list extends RecyclerView.Adapter<Adapter_Admin_Report_Diagnostics_graph_list.ViewHolder> {


    List<ModelDiagnosticReport> list = new ArrayList<>();
    List<Float> li_score_sort = new ArrayList<>();
    private Context context;


    public Adapter_Admin_Report_Diagnostics_graph_list(List<ModelDiagnosticReport> list, Context context) {
        this.list = list;
        //this.li_score_sort = li_score_sort;

        //li_score_sort.clear();
        for (int j = 0; j < list.size(); j++) {
            li_score_sort.add(Float.valueOf(list.get(j).getPercentage()));
        }
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        ...
        holder.itemBinding.tvMotivationGraphScore.setText(list.get(position).getPercentage() + "%");
