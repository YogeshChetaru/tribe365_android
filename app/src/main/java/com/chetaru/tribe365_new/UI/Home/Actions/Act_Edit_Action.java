...
public class Act_Edit_Action extends BaseActivity {
    ...
    ModelACtionList modelACtionList;
    ...
    //List<Actions> userList;
    List<ModelTheme> themeList;
    List<ModelTheme> themeListFromActionList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // setContentView(R.layout.act__edit_action);
    ...
    public void init() {
        ...
        modelACtionList = (ModelACtionList) getIntent().getSerializableExtra("model");
        getTierList();
        api_getTheme();
        tv_selecttier.setText(modelACtionList.getTier());
        tierId = modelACtionList.getTierId();
        themeListFromActionList = modelACtionList.getmThemes();

        tv_responsible_value.setText(modelACtionList.getResponsibleName());
        tv_startdate.setText(modelACtionList.getStartedDate());
        tv_duedate.setText(modelACtionList.getDueDate());
        individualUserId = modelACtionList.getResponsibleUserId();
        departmentId = modelACtionList.getOffDeptId();

        if (modelACtionList.getOrgStatus().equalsIgnoreCase("Completed")) {
            rb_complete.setChecked(true);
        }
        if (modelACtionList.getOrgStatus().equalsIgnoreCase("Not Started")) {
            rb_notstarted.setChecked(true);
        }
        if (modelACtionList.getOrgStatus().equalsIgnoreCase("Started")) {
            rb_started.setChecked(true);
        }
        et_description.setText(modelACtionList.getDescription());
    ...
    public void getTierList() {
        ...
        baseRequest.setBaseRequestListner(new RequestReciever() {
            ...
            public void onSuccess(int requestCode, String Json, Object object) {
                ...
                if (modelACtionList.getTier().equalsIgnoreCase("DEPARTMENT")) {
                    ...
                    tv_department_value.setText(modelACtionList.getOffDeptName());
                    if (departmentList.size() == 0) {
                        getDepartment();
                    }
                } else if (modelACtionList.getTier().equalsIgnoreCase("OFFICE")) {
                    tv_department.setVisibility(View.VISIBLE);
                    tv_department_value.setVisibility(View.VISIBLE);
                    tv_department.setText("RESPONSIBLE DEPARTMENT");
                    tv_department_value.setText(modelACtionList.getOffDeptName());
    ...
    @OnClick({R.id.tribe365, R.id.tv_startdate, R.id.tv_theme_value, R.id.tv_duedate, R.id.tv_selecttier, R.id.tv_responsible_value, R.id.bt_submit, R.id.tv_department_value})
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.bt_submit:
                ...
                if (status.isEmpty()) {
                    utility.showToast(mContext, "Please select status.");
                    return;
                }
    ...
    public void getDepartment() {
     ...
     //           utility.showToast(mContext, message);
       ...
       //         utility.showToast(mContext, message);
        ...
        JsonObject object = Functions.getClient().getJsonMapObject("orgId", departmentId
    ...
    public void apiUpdateAction() {
        ...
        main_object.addProperty("userId", modelACtionList.getUserId());
        main_object.addProperty("actionId", modelACtionList.getId());
        ...
        main_object.addProperty("orgId", modelACtionList.getOrgId());
...
//                "userId", modelACtionList.getUserId(),
//                "actionId", modelACtionList.getId(),
...
//                "IndividualUserId", "",
//                "orgStatus", status,
//                "orgId", modelACtionList.getOrgId() + "",
//                "description", et_description.getText().toString(),
//                "responsibleUserId", individualUserId
//
//        );
        baseRequest.callAPIPost(1, main_object,ConstantAPI.updateAction);
    }
    ...
    public void dialogTheme() {
        ...
        tv_title.setText("Select Risks");
        RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(mContext);
        rv_list.setLayoutManager(layoutManager);
        rv_list.setAdapter(new Ad_Theme(themeList, mContext));
        ...
        baseRequest.setBaseRequestListner(new RequestReciever() {
            ...
            public void onSuccess(int requestCode, String Json, Object object) {
                try {
                    ...
                    for (int i = 0; i < themeList.size(); i++) {
                        for (int j = 0; j < themeListFromActionList.size(); j++) {
                            if (themeList.get(i).getId().equals(themeListFromActionList.get(j).getId())) {
    ...
    public class Ad_Theme extends RecyclerView.Adapter<Ad_Theme.ViewHolder> {
        ...
        @Override
        public void onBindViewHolder(@NonNull final Ad_Theme.ViewHolder holder, final int position) {
            holder.tv_tier.setText(list.get(position).getTitle());
            if (list.get(position).isSelected()) {
                holder.tv_tier.setTextColor(context.getResources().getColor(R.color.white));
                holder.ll_main.setBackgroundColor(context.getResources().getColor(R.color.red));
                fab.setVisibility(View.VISIBLE);
                //list.get(position).setSelected(false);
            } else {
                holder.tv_tier.setTextColor(context.getResources().getColor(R.color.textcolor));
                holder.ll_main.setBackgroundColor(context.getResources().getColor(R.color.bg_light_grey));
            }
            holder.tv_tier.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (list.get(position).isSelected()) {
                        holder.tv_tier.setTextColor(context.getResources().getColor(R.color.textcolor));
    ...
    public int getLayoutId() {
        return R.layout.act__edit_action;
    }
